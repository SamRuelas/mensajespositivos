package com.example.mensajespositivos_10;
...
public class MainActivity extends AppCompatActivity {
    ...
    //de lo de eddy private static final String TAG = "Positive";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Switch
        switchE=(Switch) findViewById(R.id.day); //inicializamos Switch
        switchE.setText("Day/Night");
        switchE.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if(isChecked) { //NOCHE
                    constraintLayout.setBackgroundColor(Color.BLUE);//cambio color fondo
                    textView.setTextColor(Color.WHITE);
                    editText.setTextColor(Color.WHITE);
                    switchE.setTextColor(Color.WHITE);
                }else{
                    constraintLayout.setBackgroundColor(Color.WHITE);
                    textView.setTextColor(Color.BLACK);
                    editText.setTextColor(Color.BLACK);
                    switchE.setTextColor(Color.BLACK);
                }
            }
        });
        //Fondo
        constraintLayout =(ConstraintLayout) findViewById(R.id.fondo);
        ((ConstraintLayout) constraintLayout).setBackgroundColor(Color.rgb( 255, 255, 255));
        //Mensajes
        textView = findViewById(R.id.mensaje);
        textView.setText("In a world where you can be anything, be kind.");
        textView.setTextColor(Color.BLACK);

        editText = (EditText) findViewById(R.id.editText);
        //EL boton no se pone aqui porque el hace referencia a un método

        Toast.makeText(this, "Holis", Toast.LENGTH_LONG).show();
        Log.i("Positive", "WELCOME");
    ...
    public void after(android.view.View view){
        contador++;
        if(contador >=max){
            contador=min;
        }
        textView.setText(mensajes[contador]);
    }

    public void before(android.view.View view){
    ...
    public void enter(android.view.View view){
        int seleccion = Integer.parseInt(editText.getText().toString());
        if(seleccion < min){
            Toast.makeText(this, "Value unaccepted, must be greater than zero", Toast.LENGTH_LONG).show();
        }else if(seleccion >max){
                Toast.makeText(this, "Too big", Toast.LENGTH_LONG).show();
        }else{
            textView.setText(mensajes[seleccion]);
        }
